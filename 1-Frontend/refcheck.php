...
$comparisonDataPath = __DIR__ . '/data/comparison_results.json';
$comparisonData = file_exists($comparisonDataPath) ? json_decode(file_get_contents($comparisonDataPath), true) : [];

// Note: On ne pas inclure le header.php car il contient un logo encombrant
// À la place, on génère un header minimal
?>
<!DOCTYPE html>
<html lang="fr">
...
<body>
    ...
    <script>
        ...
        function displayMockResults() {
            // Charger les données réelles depuis le serveur
            fetch('./data/comparison_results.json')
                .then(response => response.json())
                .then(comparisonData => {
                    const mockData = comparisonData.map(item => ({
                        score: Math.round(item.comparison_result.final_score * 100),
                        title: item.original.title,
                        authors: item.original.authors ? item.original.authors.join(', ') : 'Inconnu',
                        year: item.original.year,
                        journal: item.original.journal,
                        doi: item.original.doi,
                        hallucination_score: Math.round(item.comparison_result.hallucination_score * 100),
                        status: item.comparison_result.status,
                        original_data: item.original,
                        comparison_data: item.comparison_result,
                        justification: item.comparison_result.status === 'OK' ?
                            `Référence vérifiée avec succès. Score: ${Math.round(item.comparison_result.final_score * 100)}%` :
                            `Attention: certains champs ne correspondent pas. Score: ${Math.round(item.comparison_result.final_score * 100)}%`
                    }));
        ...
        function displayResults(mockData) {
